'use strict';
const service = require('../services/nhapKhoSP.service');

exports.create = async (req, res) => {
  try {
    const result = await service.createNhapSP(req.body);
    res.status(201).json({ success: true, message: 'Tạo phiếu nhập SP thành công', data: result });
  } catch (err) {
...
exports.getAll = async (req, res) => {
  try {
    ...
    const list = await service.getAllNhapSP(id_dn);
...
exports.getById = async (req, res) => {
  try {
    const rec = await service.getNhapSPById(req.params.id);
...
exports.update = async (req, res) => {
  try {
    const updated = await service.updateNhapSP(req.params.id, req.body);
...
exports.delete = async (req, res) => {
  try {
    await service.deleteNhapSP(req.params.id);
    res.json({ success: true, message: 'Xóa phiếu nhập SP thành công' });
  } catch (err) {
    res.status(400).json({ success: false, message: err.message });
  }
};
