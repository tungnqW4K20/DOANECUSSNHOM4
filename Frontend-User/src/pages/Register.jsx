// ✅ Phiên bản cuối cùng: gọi API thật, giấy phép có thể null, dùng alert thay vì message
    ...
    const handleUpload = async ({ file, onSuccess, onError }) => {
        try {
            ...
            if (res?.data?.imageUrl) {
                ...
                // message.success("Tải giấy phép thành công!");
                alert("Tải giấy phép thành công!");
                onSuccess(res.data, file); // báo cho Ant Design biết upload xong
            } else {
                // message.warning("Máy chủ không trả về URL file!");
                alert("Máy chủ không trả về URL file!");
                onError(new Error("Không có URL file!")); // báo lỗi nếu không có URL
            }
        } catch (err) {
            console.error("Lỗi upload:", err);
            // message.error(err?.message || "Không thể tải file lên!");
            alert(err?.message || "Không thể tải file lên!");
    ...
    const onFinish = async (values) => {
        try {
            ...
            if (res?.success) {
                ...
                // message.warning(res?.message || "Đăng ký không thành công!");
                alert(res?.message || "Đăng ký không thành công!");
            }
        } catch (err) {
            console.error("Lỗi đăng ký:", err);
            // message.error(err?.message || "Lỗi khi đăng ký tài khoản!");
            alert(err?.message || "Lỗi khi đăng ký tài khoản!");
        }
    };
