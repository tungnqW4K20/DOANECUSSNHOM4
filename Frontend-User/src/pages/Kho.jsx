...
const Kho = () => {
  ...
  const fetchKho = async () => {
    try {
      ...
      message.error(err.message || "Không thể tải danh sách kho");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onFinish = async (values) => {
    try {
      ...
      message.error(err.message || "Lỗi khi lưu kho");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id_kho) => {
    try {
      ...
      message.success("🗑️ Xóa kho thành công!");
      await fetchKho();
    } catch (err) {
      message.error(err.message || "Lỗi khi xóa kho");
    } finally {
      setLoading(false);
    }
  };
