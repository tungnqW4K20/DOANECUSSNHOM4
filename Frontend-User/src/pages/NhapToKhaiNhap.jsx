...
const NhapToKhaiNhap = () => {
    ...
    const [chiTietHoaDon, setChiTietHoaDon] = useState([{ key: 1, id_npl: null, so_luong: 0, don_gia: 0, tri_gia: 0 }]);
    ...
    const handleAddRow = () => setChiTietHoaDon([...chiTietHoaDon, { key: Date.now(), id_npl: null, so_luong: 0, don_gia: 0, tri_gia: 0 }]);
    ...
    const onFinish = async () => {
        try {
            ...
            message.success('Nộp tờ khai nhập thành công!');
        } catch (error) {
    ...
    const columnsChiTiet = [
        { title: 'Nguyên phụ liệu', dataIndex: 'id_npl', render: (_, record) => (
            <Select style={{ width: 200 }} onChange={(val) => handleChiTietChange(record.key, 'id_npl', val)} placeholder="Chọn NPL">
    ...
    const steps = [
        ...
        { title: '2. Hóa đơn & Vận đơn', content: (
            <Form form={formHoaDonVanDon} layout="vertical" name="form_hd_vd">
                <Title level={5}>Thông tin Hóa đơn nhập</Title>
                <Row gutter={24}>
                ...
                <Button onClick={handleAddRow} type="dashed" icon={<PlusOutlined />} style={{marginTop: 16, width: '100%'}}>Thêm Nguyên phụ liệu</Button>
                
                <Title level={5} style={{marginTop: 24}}>Thông tin Vận đơn nhập</Title>
                <Row gutter={24}>
    ...
    return (
        <>
            <Title level={3} style={{ textAlign: 'center', marginBottom: 24 }}>Khai báo Tờ khai Nhập khẩu</Title>
            <Card>
