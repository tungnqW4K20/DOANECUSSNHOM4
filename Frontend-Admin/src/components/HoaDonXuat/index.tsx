...
import { mockData_HoaDonXuat } from './components/mockData';
import { HoaDonXuat_Colum } from './components/table';
import { HoaDonXuat_Form } from './components/form';


const HoaDonXuatComponents = () => {
  const [data, setData] = useState<GetHoaDonXuat_DTO[]>([]);
  const [loading, setLoading] = useState(false);
  const [modalVisible, setModalVisible] = useState(false);
  const [editingItem, setEditingItem] = useState<GetHoaDonXuat_DTO | null>(null);
  ...
  useEffect(() => {
    document.title = 'Quản lý Hóa đơn xuất';
    GetData(currentPage, pageSize, searchValue);
  }, [currentPage, pageSize, searchValue]);

  const GetData = async (pageIndex: number, pageSize: number, keyword?: string) => {
    setLoading(true);
    let filtered = mockData_HoaDonXuat;
    if (keyword) filtered = filtered.filter(x => x.so_hd.toLowerCase().includes(keyword.toLowerCase()));
  ...
  const openEditModal = (record: GetHoaDonXuat_DTO) => { setEditingItem(record); setIsEditing(true); form.setFieldsValue(record); setModalVisible(true); };
  const closeModal = () => { setModalVisible(false); setEditingItem(null); setIsEditing(false); form.resetFields(); };
  const handleDelete = (record: GetHoaDonXuat_DTO) => { console.log('Xóa:', record); show({ result: 0, messageDone: `Đã xóa hóa đơn #${record.id_hd_xuat}` }); };
  ...
  const columns = COLUMNS({ columnType: HoaDonXuat_Colum, openModal: openEditModal, handleDelete });

  return (
    <>
      <Header_Children title="Quản lý Hóa đơn xuất" onAdd={openCreateModal} text_btn_add="Thêm hóa đơn mới" />
      <Divider />
      ...
      <div className="py-4" style={{ marginTop: 20 }}>
        <Table columns={columns} dataSource={data} rowKey="id_hd_xuat" loading={loading} scroll={{ x: 1200, y: 400 }} pagination={{
      ...
      {modalVisible && (
        <Modal title={isEditing ? 'Cập nhật hóa đơn' : 'Thêm hóa đơn'} open={modalVisible} onOk={handleSave} onCancel={closeModal} width="60%" centered okText="Lưu" cancelText="Hủy">
          <HoaDonXuat_Form formdata={form} isEditing={isEditing} />
        </Modal>
      )}
    </>
  );
...
export default HoaDonXuatComponents;
